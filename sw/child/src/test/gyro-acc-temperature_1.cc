/******************************************************************************/
/*! @brief      mxfs filesystem driver implementation of test operation(gyro-acc-tempereture-1)
...
TEST(Basic, Gyro_Acc_Temp_1) {
    ...
    for(int n = 0;n < timerange_count;n++) {
        TEST_INFO(
            "temperature: timerange(%d, %u, %u, %u, %u)",
            n,
            timerange[n].block,
            timerange[n].page,
    ...
    ASSERT_EQ(readlen, sizeof(compare[0]));
